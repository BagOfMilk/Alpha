...
**Ночь** — та же цепочка, но: производство и лечение отключены **полностью**
(не «частично», как было в этом черновике), реплики горожан недоступны
(диалоги закрыты), зато срабатываний до двух и доступен канал `Ambient`.

> Уточнено 2026-09-20 при написании моста. Календарные сутки — это две фазы,
> и «частично» пришлось бы выражать долей выработки: неочевидное число там, где
> US-1.5 говорит прямо «ночью назначения на позиции закрыты», а GDD Э1 —
> «патрулировать без лечения». Шаг производства просто не выполняется ночью,
> и это проверяется тестом `CalendarDay_ProducesExactlyOnce`.

```
DayReport
    Day, Phase
    // CycleReport Production — УСТАРЕЛО, так не сделано.
    // DayReport лежит в Core/Loop и потому не может нести ResourceType:
    // числа производства идут отдельным каналом ProductionStep.LastReport.
