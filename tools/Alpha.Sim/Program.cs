    ...
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            foreach (var policy in policies)
            {
                ...
                // FastConstruction = false: цей харнес калібрує баланс кампанії
                // (тир/темп/economy на 90 діб), тож стройка має йти за проектними
                // BuildingDefinition.Days (Steward/CityWorksTests бачать те саме,
                // бо кличуть CityWorks.Order напряму). Без цього поле відкату
                // до дефолту NewGameOptions.FastConstruction=true (Поправка №7.7,
                // тестова сборка) мовчки підмінило б усі строки стройки на одну
                // добу — саме тому, що BotRunner.PlayDays веде сесію через той
                // самий GameSession, що й tools/Alpha.Play.
                var options = new NewGameOptions { SkipCreation = false, HitRule = HitRuleKind.Threshold, Seed = seed, Roller = roller, FastConstruction = false };
