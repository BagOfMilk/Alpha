...
{
    /// <summary>
    /// Сборка играбельного билда для ПЛЕЙТЕСТА (docs/PLAYTEST.md). Тестер не должен
    /// ставить Unity: ему нужен каталог с .exe, который запускается двойным кликом.
    /// Запуск: меню Alpha → Build Windows, или batchmode
    /// -executeMethod Game.EditorTools.Builder.BuildWindows (см. tools/build.ps1).
    /// Сцены берутся из Build Settings — точка входа та же, что в редакторе
    /// (Campaign, индекс 0), иначе билд показал бы тестеру не ту игру.
    /// </summary>
    public static class Builder
    {
        ...
        [MenuItem("Alpha/Build Windows")]
        public static void BuildWindows()
        {
            var scenes = EnabledScenes();
            if (scenes.Length == 0)
            {
                Fail("В Build Settings нет включённых сцен — сначала Alpha → Create All Scenes");
                return;
            }
