...
{
    /// <summary>
    /// Боевой ИИ (итерация 15): достойный, ЧИТАЕМЫЙ соперник — не оптимальный.
    /// Utility-скоринг: цель по ожидаемому урону × шансу (+добивание, +Метка),
    /// позиция по роли (US-3.13: клинч-роли сближаются, стрелки держат оптимал и
    /// укрытие), способности из общего пула по простым приоритетам (рывок вне
    /// контакта → свежий статус → усиленный залп). Side-агностичен: тем же кодом
    /// ходят враги, босс-перебежчик и авто-союзники. Детерминирован при данном
    /// CombatState (вся случайность — внутри бросков атаки).
    /// </summary>
    public static class CombatAi
    {
        ...
        /// <summary>Прогоняет бой ИИ-против-ИИ до исхода (симуляции/тесты). Бюджет страхует.</summary>
        public static void AutoResolve(CombatState cs, int turnBudget = 200)
        {
        ...
        private static bool TryAct(CombatState cs, CombatUnit unit)
        {
            ...
            // 5. Свежий статус на цель (Подавление/Метка/Оглушение — характер Контролёра,
            //    но полезен всем): только если цель ещё не под ним.
            ...
            {
                bool strike = unit.StrikeMeter >= cs.Balance.StrikeGuaranteeAt;
        ...
        private static CombatUnit PickTarget(CombatState cs, CombatUnit unit)
        {
            ...
            foreach (var t in cs.Units)
            {
                ...
                else
                {
                    ...
                    // Непростреливаемая цель не должна перебивать легальный выстрел
                    // (штраф больше суммы бонусов добивания и Метки).
        ...
        private static double TileScore(CombatState cs, CombatUnit unit, GridPos pos, CombatUnit target)
        {
            ...
            if (closeCombat)
                return -dist * 2.0; // клинч-роли сближаются, укрытия им безразличны (US-3.13)

            // Стрелок/контролёр: держать оптимал, ценить укрытие ПРОТИВ цели, не терять LOS.
        ...
        private static AbilityDefinition PickStatusAbility(CombatState cs, CombatUnit unit, CombatUnit target)
        {
            foreach (var a in unit.Abilities)
            {
                ...
                foreach (var fx in a.Effects)
                {
                    ...
                    return a; // свежий статус — всегда осмысленный ход
                }
            }
            return null;
        }
        ...
        private static AbilityDefinition PickVolleyAbility(CombatUnit unit)
        {
            foreach (var a in unit.Abilities)
            {
                ...
                if (weaponHits >= 2 && !other) return a; // чистый «усиленный залп» (Очередь)
            }
            return null;
        }
