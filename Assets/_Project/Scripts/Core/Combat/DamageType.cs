...
{
    /// <summary>
    /// Лёгкая ось типов урона: множитель на попадании по резист/уязвимости
    ...
    public enum DamageType
    {
        ...
        Ballistic = 1, // баллистический (стрелы, метательное)
        Fire = 2,      // огонь (может накладывать Поджог)
        Toxin = 3,     // токсин (может накладывать Яд)
        Energy = 4     // энергия (ПЛЕЙСХОЛДЕР под будущий контент)
    }

    /// <summary>
    /// Профиль резист/уязвимость юнита: множитель урона per тип. Нет записи = ×1.
    /// Резист ×0.5–0.75, уязвимость ×1.25–1.5 (ПЛЕЙСХОЛДЕРЫ, крутятся балансом).
    /// </summary>
    [Serializable]
