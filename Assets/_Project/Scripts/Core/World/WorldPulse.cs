...
namespace Game.Core.World
{
    ...
    public sealed class WorldPulse
    {
        ...
        /// Threshold ОДНИМ стрибком BoostCharge означало б: WorldPulse.
        /// Fire() тут-таки, в тому самому виклику Advance(), мовчки скидає
        ...
        ///
        /// НЕ закрито цим клямпом (задокументовано, не приховано):
        /// звичайне щоденне накопичення Тугара (тіка кожну фазу — §1
        /// рядок 8 табл. TEST_BUILD.md) саме по собі перетинає Threshold=60
        /// близько доби 3 навіть БЕЗ жодного гравецького предмета — це вже
        /// існуючий, не внесений цим пакетом дефект (Тугар ще не має
        /// власного IncidentDefinition), і клямп однієї точки входу
        /// (BoostCharge) його не усуває — для цього потрібен або реальний
