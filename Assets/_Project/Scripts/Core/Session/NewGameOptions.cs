...
namespace Game.Core.Session
{
    ...
    public sealed class NewGameOptions
    {
        ...
        /// <summary>
        /// Поправка №7.7 (рішення власника 24.09.2026): у тестовій сборці кожне
        /// здание будується рівно одну добу — інакше механіка «стройка → пост →
        /// крафт/ефект» не встигає дійти до гравця за короткий тест-прогін (5–15
        /// діб), як показав GAP crafту (§6.1 №22: Майстерня в бот-прогонах не
        /// добудовується взагалі — вона остання в BotRunner.BuildPriority, а
        /// проектні строки (Поправка №6.1, ПЛЕЙСХОЛДЕР) сумарно ~35 діб на всі
        /// шість будівель послідовно). Дефолт — true, бо на цій гілці GameSession
