...
namespace Game.Core.Session
{
    ...
    public sealed class NewGameOptions
    {
        ...
        /// збірці кожна будівля будується рівно одну добу — замовлення
        /// уранці, готово до наступного ранку, пост відкривається одразу.
        ///
        ...
        /// це баланс кампанії, а не тестової збірки. Саме тому
        /// <c>tools/Alpha.Sim/Program.cs</c>, який калібрує баланс кампанії,
        /// явно ставить це поле в <c>false</c> у своєму
        /// <see cref="NewGameOptions"/> — інакше калібрування мовчки їхало б
        /// на тестовому одноденному будівництві.
        /// </summary>
        public bool TestBuildOneDayConstruction = true;

        /// <summary>
        /// Поправка №7 (рішення власника 24.09.2026, дослівно: «За 5 днів
        /// пожежа, потім інші механіки. Так для тесту ввімкни що за 15 днів
        /// перша смуга за 20 друга і за 25 криза?»): тестова збірка стискає
        /// фоновий темп шкали Напруги (<see cref="Game.Core.Session.TestBuildTensionPace"/>),
        /// щоб один безперервний прогін показав і зміну смуги, і природний
        /// великий кризис — а не лишався в Спокої всі 25+ діб.
        ///
        /// Дефолт — true з тієї самої причини, що й <see cref="TestBuildOneDayConstruction"/>
        /// вище: усі три шляхи в тестову збірку (Unity, Alpha.Play, боти) йдуть
        /// крізь <see cref="GameSession.NewGame"/> без явного значення цього
        /// поля. <c>tools/Alpha.Sim/Program.cs</c>, який калібрує баланс
        /// кампанії (90 діб, п'ять політик), ставить це поле в <c>false</c>
        /// явно — інакше калібрування мовчки поїхало б на стисненій шкалі.
        /// </summary>
        public bool TestBuildTensionPace = true;
    }
