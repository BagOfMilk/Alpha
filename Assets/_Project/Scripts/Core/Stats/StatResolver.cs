...
{
    /// <summary>
    /// Единый агрегатор модификаторов (US-18.2). Ровно одно место, где числа
    /// персонажа складываются — поэтому двойной счёт невозможен структурно, а не
    /// по дисциплине: каждый эффект приходит одним StatModifier от одного
    /// провайдера.
    ///
    /// Порядок свёртки (зафиксирован тестом):
    ///     (база + Σ Flat) × (1 + Σ PercentAdd) × Π Multiplier
    /// Override перебивает всё. Порядок вставки на результат не влияет.
    /// </summary>
            ...
            for (int i = 0; i < mods.Count; i++)
            {
                ...
                switch (m.Mode)
                {
                    ...
                    case ModMode.Override:
                        // Последний Override выигрывает — но порядок провайдеров
                        // стабилен, поэтому результат воспроизводим.
                        hasOverride = true;
