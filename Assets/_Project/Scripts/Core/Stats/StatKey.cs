...
{
    /// <summary>
    /// Единственный ключ агрегатора модификаторов (US-18.2).
    ///
    /// Модель персонажа читается тремя осями (атрибуты, скилы, производные), но
    /// СКЛАДЫВАЕТСЯ всё по одному плоскому ключу. Это не противоречие: разные
    /// языки для авторства и для арифметики. Плоский enum выбран потому, что в
    /// инспекторе Unity модификатор должен быть одним дропдауном, а не парой
    /// полей с кастомным драйвером (US-18.1).
    ///
    /// Диапазоны: 100+ атрибуты, 200+ скилы, 300+ производные. Рассинхрон между
    /// осью и ключом ловится тестом StatKeyMappingTests, а не дисциплиной.
    /// </summary>
    public enum StatKey
    {
        None = 0,

        // Атрибуты: 100 + значение AttributeType
        Strength = 101,
        Agility = 102,
        Wits = 103,
        Will = 104,

        // Скилы: 200 + значение SkillType
        Ranged = 201,
        Melee = 202,
        Tactics = 203,
        Lockpick = 210,
        ...
        // Производные: 300 + значение DerivedStat
        MaxHp = 301,
        MaxAp = 302,
        Accuracy = 303,
        Defense = 304,
