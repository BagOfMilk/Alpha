...
namespace Game.Tests.EditMode
{
    ...
    public class ChroniclePresetTests
    {
        ...
        {
            var run = QuietHamlet();
            // Инвариант: без тяжёлых решений хутор не доходит до кризисной
            // территории — источник кризиса копит только с «Накала» (Heat).
            ...
            // Верхняя граница перекалибрована при слиянии линии ремонта лупа
            // (20.09.2026): починенный календарь считает сутки, а не фазы, плюс
            // подавление повторов поменяло состав исходов — тот же пресет теперь
            // оседает на «Брожении» (15 происшествий), а не на «Ропоте» (13).
            // Ослаблена ровно калибровка, инвариант выше — нет.
