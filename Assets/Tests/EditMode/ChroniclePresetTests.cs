...
{
    /// <summary>
    /// Три пресети зі сцени «Хроніка». Демо обіцяє гравцю тезу:
    /// правила одні, а доля різна — її визначають рішення, а не кубик.
    /// Тут ця обіцянка перевіряється, а не береться на віру: якщо числа
    /// пресетів підібрані погано, тести впадуть і демо не збреше власнику.
    ///
    /// Налаштування зобов'язані збігатися з ChronicleSceneBuilder.AddPreset.
    /// </summary>
    public class ChroniclePresetTests
    {
        ...
        {
            var run = QuietHamlet();
            // Інваріант: без важких рішень хутір не доходить до кризової
            // території — джерело кризи копить лише з «Розпалу» (Heat).
            Assert.AreEqual(0, run.Crises, "И кризиса там быть не должно");
            Assert.Less((int)run.FinalBand, (int)TensionBand.Heat,
                "Хутор, где игрок ничего тяжёлого не решал, не имеет права дойти до Накала");

            // Верхня межа перекалібрована при злитті лінії ремонту лупу
            // (20.09.2026): полагоджений календар рахує доби, а не фази, плюс
            // придушення повторів змінило склад підсумків — той самий пресет тепер
            // осідає на «Броженні» (15 подій), а не на «Ропоті» (13).
            // Ослаблена рівно калібровка, інваріант вище — ні.
            Assert.LessOrEqual((int)run.FinalBand, (int)TensionBand.Ferment,
                "и не имеет права уйти выше «Брожения»");
        }
        ...
        {
            // Слобода спить (patrol: false) — і платить за це сліпотою.
