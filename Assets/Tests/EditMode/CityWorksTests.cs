...
{
    /// <summary>
    /// Місто відповідає (Поправка №6).
    ///
    /// Замір харнеса до цієї роботи: шістнадцять кампаній із шістнадцяти
    /// закінчувалися Розколом, Напруга піднімалася на 680–1010 і опускалася на 8.
    /// Процвітання було прив'язане до тіру, який ніхто не змінював; населення не
    /// росло; чотири понижувальні драйвери з п'яти не викликалися ніде. Тут
    /// закріплено, що в міста з'явилося чим відповісти — і що кожна відповідь
    /// коштує ціни, як вирішив власник: «усі мають шось коштувати».
    /// </summary>
    public class CityWorksTests
    {
        ...
        {
            // Просунуті будівлі (GDD US-7.1) коштують будівельного компонента,
        ...
        /// <c>BuildingDefinition.Days</c> БУДЬ-ЯКОЇ будівлі — навіть Храму
        /// (Days=8), який роками не встигав до кінця короткого тестового
        /// прогону.
        /// </summary>
        [Test]
        ...
        public void Tier_ShowsOnTheMoodboard()
        {
            ...
            // Той самий звіт, що оголосив ріст, вже показує село. Плівка діб
            // спіймала відставання: у стрічці «хутір став селом», а в заголовку кадру
            // ще «хутір» — мудборд збирався зі вчорашнім тіром.
        ...
        // ================= люди приходять по події і з вилазки =================
        ...
        /// <summary>
        /// Будівля відкриває пост, але не займає його. Плівка діб знайшла це
        /// раніше тестів: лазарет стояв з четвертих діб, а вночі «на посту
        /// нікого не було» — лікар сидів без діла. Господар зобов'язаний поставити на
        /// відкритий пост найкращого вільного за профільною навичкою — і не
        /// чіпати зайнятих, мертвих, тих, хто пішов у вилазку, і тих, хто в цій справі не тямить.
        /// </summary>
        [Test]
        public void Steward_StaffsAnOpenedPost_WithTheBestFreeHand()
        {
            ...
            // Лазарета нема — ставити нікуди: поле і склад лікарям не за навичкою.
            ...
            // Повтор нічого не змінює: детермінізм і ідемпотентність.
        ...
        /// <summary>
        /// Приймання Поправки №6.5. До неї будь-яка кампанія закінчувалася Розколом: у
        /// лупа був тиск і не було клапана. Тут одне й те саме місто живе
        /// дев'яносто діб двічі — без господаря і з рачительним господарем, який
        /// будує, кличе облаву і приймає людей. Гаманець в обох однаковий:
        /// різниця лише в тому, чи грає хтось.
        /// </summary>
        [Test]
        public void Valve_GoodPlay_MakesADifference()
        {
            ...
            // Темп дуги (Поправка №6.4): село — близько тридцятих діб. Вікно
            // широке, поки числа плейсхолдерні, але нижня межа — захист від
            // регресії: без відкату прийому переселенців хутір ставав селом
            // на одинадцяту добу, і це було видно тільки на плівці.
        ...
        private static City FullWorld(BalanceConfig cfg)
        {
            ...
            // Однаковий стартовий гаманець в обох: порівнюється гра, а не удача.
