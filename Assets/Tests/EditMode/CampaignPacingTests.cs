...
namespace Game.Tests.EditMode
{
    ...
    public class CampaignPacingTests
    {
        ...
        /// <summary>
        /// ПЕРЕМЕРЕНО (полірування, ціль 1 «Картка персонажа»): було — 0;
        /// стало — 2. Причина НЕ в том, что разрыв открылся заново кодом
        /// конвейера: <c>ForceSignalOnBandChange</c> не трогался. Причина —
        /// Core/Characters/Traits/DefaultTraits.cs (новый контент, впервые
        /// даёт стартовым трейтам именного каста реальные экземпляры —
        /// раньше <c>TraitDefinition</c> существовал без единого случая
        /// использования) добавил модификаторы скилов пятерым именным NPC
        /// (напр. Meticulous: Keeper +1 Trade/-1 Persuade), и в ИМЕННО ЭТОМ
        /// измеренном наборе кампаний это честно сдвинуло исходы 2 соц-проверок
        /// через порог — так же, как раньше это сделал G16. Разрыв инварианта 4
        /// остаётся открытым архитектурно (см. текст ниже) — тест держит
        /// ЧЕСТНОЕ измеренное число, а не подгоняет его под ожидание.
        /// </summary>
        [Test]
        public void Pacing_BandChangeCanBeMute_KnownGap()
        {
            ...
            Assert.AreEqual(2, mute,
                ...
                "не закрыт кодом — число честно обновлено под текущий контент, а не подогнано под ожидание");
        }
