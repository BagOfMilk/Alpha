...
namespace Game.Tests.EditMode
{
    ...
    public class CampaignPacingTests
    {
        ...
        public void Pacing_OneTopicDominatesTheWholeCampaign_KnownGap()
        {
            foreach (var m in All())
            {
                Assert.GreaterOrEqual(m.MaxTopicRepeats, 100,
                    Where(m) + " — ИЗВЕСТНЫЙ РАЗРЫВ: самый частый ключ реплики показывается " +
                    m.MaxTopicRepeats + " раз за кампанию при " + m.DistinctTopics +
                    " уникальных ключах всего. Анти-повтора в композиторе нет");
