...
namespace Game.Tests.EditMode
{
    ...
    public class CampaignPacingTests
    {
        ...
        /// <summary>
        /// ПЕРЕМЕРЕНО (B7, G16 — атрибут в соц-проверках, аудит `docs/AUDIT-GAPS.md`):
        /// было — 12×200 суток давали хоть одну немую смену полосы (mute &gt; 0);
        /// стало — 0. Причина НЕ в том, что разрыв закрыт кодом: <c>ForceSignalOnBandChange</c>
        /// по-прежнему только добавляет кандидата и не резервирует слот бюджета
        /// сигналов (см. текст ниже) — этот код в пакете B7 не трогался вообще.
        /// Причина в том, что G16 (Убеждение/Торговля добирают Смекалку,
        /// Запугивание — Волю, GDD:98) честно сдвинул исходы соц-проверок по
        /// именному касту <c>FirstHourWorld</c>, и в ИМЕННО ЭТОМ измеренном
        /// наборе кампаний шумных дней, где бюджет сигналов переполнен, больше
        /// не случилось. Разрыв инварианта 4 остаётся открытым архитектурно —
        /// другой баланс/контент легко воспроизведёт mute &gt; 0 снова; тест
        /// держит ЧЕСТНОЕ измеренное число, а не подтверждает, что гарантия
        /// инварианта 4 теперь безусловна.
        ///
        /// ПЕРЕМЕРЕНО (накопитель без своего инцидента больше не сгорает молча,
        /// 24.09.2026): было 0 — стало 1, AggressiveChoices, тир 2, сутки 40,
        /// Накал → Излом. Предсказание абзаца выше сбылось: код сигналов снова
        /// не трогался, сдвинулся контент кампании. Раньше «Тугар» (у него нет
        /// инцидента) на 39-е сутки разряжался вхолостую и забирал единственный
        /// слот дня; теперь слот достаётся «улице», её missing_person с Худшим
        /// исходом дотягивает Напряжение до Излома на 40-е сутки, а в тот день
        /// бюджет занят жителями, двумя докладами постов и первой ступенью
        /// кризиса — реплике о смене полосы места нет.
        /// </summary>
        [Test]
        public void Pacing_BandChangeCanBeMute_KnownGap()
        {
            ...
            Assert.AreEqual(1, mute,
                "ИЗВЕСТНЫЙ РАЗРЫВ (инвариант 4: принудительный сигнал только ДОБАВЛЯЕТ кандидата, " +
                "слота не резервирует, поэтому в шумный день его вытесняет бюджет) не закрыт кодом. " +
                "Измерено: одна немая смена полосы на 12×200 суток (AggressiveChoices, тир 2, сутки 40) — " +
                "число честно обновлено, а не подогнано под ожидание");
        }
