    ...
    public class CombatOverwatchTests
    {
                                    ...
                                    int acc = 85, WeaponDefinition w = null)
        {
            var p = new UnitProfile
            {
                ...
                MedicineSkill = 0, CanBeDowned = false, MoveApPerTile = 1
            };
        ...
        public void Overwatch_SeesRepositionByOrder_IntoSector()
        {
            ...
            leader.Abilities.Add(DefaultCombatContent.MoveOrder());
            ...
            Assert.AreEqual(CombatActionResult.Success,
                cs.UseAbility("ability.move_order", "ally", new GridPos(7, 4)));
        ...
        public void Overwatch_BeforeTrap_OnMove_FallenUnitDoesNotSpringIt()
        {
            ...
            watcher.Abilities.Add(DefaultCombatContent.SetTrap());
            Assert.AreEqual(CombatActionResult.Success, cs.UseAbility("ability.set_trap", null, new GridPos(2, 4)));
            cs.Overwatch(new GridPos(9, 2));
        ...
        public void Overwatch_BeforeTrap_OnLunge_SameOrderAsMove()
        {
            ...
            watcher.Abilities.Add(DefaultCombatContent.SetTrap());
            lunger.Abilities.Add(DefaultCombatContent.Lunge());
            cs.AddUnit(watcher, new GridPos(0, 2));
            cs.AddUnit(lunger, new GridPos(0, 4));
            cs.AddUnit(bait, new GridPos(3, 4));
            cs.Begin();
            Assert.AreEqual(CombatActionResult.Success, cs.UseAbility("ability.set_trap", null, new GridPos(2, 3)));
            cs.Overwatch(new GridPos(9, 2));

            Assert.AreSame(lunger, cs.Current);
            Assert.AreEqual(CombatActionResult.Success, cs.UseAbility("ability.lunge", "bait"));
        ...
        public void Overwatch_Shot_HasPenalty_MatchesPreview_AndFeedsNoStrike()
        {
            ...
            Assert.AreEqual(85 - Cfg.Combat.OverwatchAccuracyPenalty, shot.Chance, "навскидку — со штрафом");
            Assert.AreEqual(cs.OverwatchHitChancePreview(watcher, enemy), shot.Chance,
                "показанное число обязано совпадать с фактическим роллом (телеграфия)");
